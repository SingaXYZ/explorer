...
export const useBaseStore = defineStore('baseStore', {
  ...
  actions: {
     ...
     * older blocks are fetched concurrently (bounded by SEED_CONCURRENCY). Arrivals are
     * buffered and merged into recents once per animation frame, so blocks that land in
     * the same frame render together in a single batch instead of one at a time — the
     * concurrency shows up as batches of blocks appearing, not a serial trickle. recents
     * is kept in ascending height order (deduped, capped). Failed heights are skipped.
     */
    async seedRecentBlocks(): Promise<void> {
      ...
      // Show the latest block right away; older blocks fill in as batches arrive.
      ...
      // Buffer arrivals and merge them into recents in one reactive update per frame, so
      // concurrent fetches that return together produce a single render, not N renders.
      const pending: Block[] = [];
      ...
      flush(); // flush anything buffered after the last frame
    },
